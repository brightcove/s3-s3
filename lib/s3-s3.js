    ...
    Request = function(realRequest) {
      ...
      return {
        ...
        send: function() {
          ...
          // wherever possible
          realRequest.on('success', function(response) {
            if (onFunc.hasOwnProperty('success')) {
              onFunc.success(response);
            }
          });
          realRequest.on('error', function(response) {
            if (onFunc.hasOwnProperty('error')) {
              onFunc.error(response);
            }
          });
          realRequest.send();
        }
